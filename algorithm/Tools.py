...
class Dataset:
    def __init__(self, increased_ys=True, scale_data=False):
...
def pca_input(num_features, matrix):
    ...
    return matrix_projected, eigenvalues, eigenvectors
...
def mean_absolute_percentage_error(y_true, y_pred):
    lista = []
    for num_prediction, prediction in enumerate(y_pred):
        new_result = (abs(y_true[num_prediction] - prediction)) / y_true[num_prediction]
        lista = np.append(lista, new_result)
    mape = np.sum(lista) / len(lista) * 100
    return mape
...
class Models:
    ...
    def train_model(self, clf):
        ...
        if self.k_fold is False:
            ...
            kf = KFold(n_splits=constant.num_splits_Kfold, random_state=10, shuffle=True)
            predictions = []
            tests = []
    ...
    def evaluate_model(self, testY, predY, train_model=True):
        if self.k_fold:
            r2 = self.obtain_mean(r2_score, testY, predY)* 100
            accuracy = 0
...
class LearningCurve:
    def __init__(self, clf, X, Y, score, train_size, id_test):
        self.clf = clf
        self.X = X
        self.Y=Y
        self.score = score
...
class Plots:
    ...
    def unsupervised_models(self, matrix, labels):
        ...
        print(variance_ratio_matrix)

        ##### ICA ####
        ica = ICA(n_components=2, random_state=1)
